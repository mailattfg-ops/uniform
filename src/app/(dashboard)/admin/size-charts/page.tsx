            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <Select
                  ...
                  disabled={!!editingId}
                  onChange={(val) => {
                    const cat = val as 'top_wear' | 'bottom_wear';
                    setNewChart({
                      ...newChart,
                      category: cat,
              ...
              <div className="space-y-4">
                ...
                <div className="flex bg-zinc-50 rounded-2xl p-1 h-14">
                  <Button
                    ...
                    variant="secondary"
                    size="h-auto"
                    className={`flex-1 rounded-xl text-[10px] font-black uppercase transition-all border-none shadow-none ${newChart.unit === 'cm' ? 'bg-[#2d8d9b] text-white shadow-md' : 'text-zinc-400 bg-transparent'}`}
                  >
                    cm
                  </Button>
                  <Button
                    ...
                    variant="secondary"
                    size="h-auto"
                    className={`flex-1 rounded-xl text-[10px] font-black uppercase transition-all border-none shadow-none ${newChart.unit === 'in' ? 'bg-[#2d8d9b] text-white shadow-md' : 'text-zinc-400 bg-transparent'}`}
                  >
                    in
                  </Button>
                </div>
          ...
          <Card key={gIdx} className="p-10 border-none shadow-2xl rounded-[3rem]">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4 flex-1 max-w-sm">
                ...
                <Input
                  className="bg-transparent border-b-2 border-dashed border-zinc-200 focus:border-[#2d8d9b] text-sm font-black text-[#3a525d] pb-1 w-full rounded-none"
                  value={group.label}
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="secondary"
                  size="h-auto"
                  className="w-8 h-8 rounded-lg bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all border-none shadow-none p-0"
                >
                  <Trash2 size={12} />
                </Button>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-4 text-[#333]">
              {group.data.map((d, i) => (
                <div key={i} className="space-y-3 relative group">
                  <Input
                    className="text-[10px] font-black text-center block w-full bg-transparent border-none text-zinc-400 uppercase rounded-none px-0 py-0 shadow-none"
                    value={d.size}
                    onChange={e => updateSizeLabel(gIdx, i, e.target.value)}
                    allowSpecialCharacters
                  />
                  <Input
                    className="w-full h-12 bg-zinc-50 border-2 border-transparent focus:border-[#2d8d9b] rounded-xl text-center text-xs font-black transition-all shadow-sm px-2"
                    placeholder="Range"
                    value={d.value}
                  ...
                  <Button
                    ...
                    variant="secondary"
                    size="h-auto"
                    className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity border-none shadow-none p-0"
                  >
                    <X size={10} />
                  </Button>
                </div>
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-[#fce4d4] shadow-sm">
          <Button
            ...
            variant="secondary"
            size="h-auto"
            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-none shadow-none ${unit === 'cm' ? '!bg-[#f2994a] text-white shadow-lg' : 'text-[#3a525d]/40 !bg-transparent'}`}
          >
            cm
          </Button>
          <Button
            ...
            variant="secondary"
            size="h-auto"
            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-none shadow-none ${unit === 'in' ? '!bg-[#f2994a] text-white shadow-lg' : 'text-[#3a525d]/40 !bg-transparent'}`}
          >
            in
          </Button>
        </div>
      ...
      <div className="flex gap-4 border-b border-zinc-100 uppercase text-[11px] font-black tracking-[0.2em] pb-0">
        <Button
          ...
          variant="secondary"
          size="h-auto"
          className={`pb-4 px-4 transition-all relative border-none shadow-none bg-transparent rounded-none ${activeTab === 'top_wear' ? 'text-[#2d8d9b]' : 'text-zinc-400 hover:text-zinc-600'}`}
        >
        ...
        <Button
          ...
          variant="secondary"
          size="h-auto"
          className={`pb-4 px-4 transition-all relative border-none shadow-none bg-transparent rounded-none ${activeTab === 'bottom_wear' ? 'text-[#2d8d9b]' : 'text-zinc-400 hover:text-zinc-600'}`}
        >
                      ...
                      {idx === 0 && (
                        <div className="flex gap-2">
                          <Button
                            ...
                            variant="secondary"
                            size="h-auto"
                            className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-all text-white border-none shadow-none p-0"
                          >
                          ...
                          <Button
                            ...
                            variant="secondary"
                            size="h-auto"
                            className="w-8 h-8 rounded-lg bg-red-500/20 text-red-100 flex items-center justify-center hover:bg-red-500/30 transition-all border-none shadow-none p-0"
                          >
                            <Trash2 size={12} />
                          </Button>
                        </div>
            ...
            <div className="flex items-center gap-2">
              {[...Array(totalPages)].map((_, i) => (
                <Button
                  ...
                  variant="secondary"
                  size="h-auto"
                  className={`w-10 h-10 rounded-xl font-black text-xs transition-all border-none shadow-none ${currentPage === i + 1 ? 'bg-[#2d8d9b] text-white shadow-lg' : 'bg-zinc-100 text-zinc-400 hover:bg-zinc-200'}`}
                >
                  {i + 1}
                </Button>
              ))}
