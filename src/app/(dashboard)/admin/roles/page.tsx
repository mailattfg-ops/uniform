...
const AVAILABLE_PERMISSIONS = [
  ...
  { id: 'view_products', label: 'View Product Registry', category: 'Catalog' },
  { id: 'manage_products', label: 'Manage Product Registry', category: 'Catalog' },
  ...
  const columns: Column<Role>[] = [
    {
      ...
      accessor: (r) => (
        <div className="flex items-center gap-4 py-2">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-inner ${r.name === 'Admin' ? 'bg-[#3a525d] text-white' : 'bg-[#2d8d9b]/10 text-[#2d8d9b]'
            }`}>
                    ...
                    <div key={category} className="space-y-4">
                      ...
                      <div className="space-y-2 pl-2">
                        {AVAILABLE_PERMISSIONS.filter(p => p.category === category).map(permission => (
                          <label
                            key={permission.id}
                            className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer group ${formData.permissions.includes(permission.id)
                              ? 'bg-[#2d8d9b]/5 border-[#2d8d9b]/30'
                              : 'bg-white border-zinc-100 hover:border-zinc-200'
                              }`}
                          >
                            <div className="flex flex-col gap-1">
                              <span className={`text-[11px] font-black transition-colors ${formData.permissions.includes(permission.id) ? 'text-[#2d8d9b]' : 'text-zinc-500'
                                }`}>
                                {permission.label}
                              </span>
