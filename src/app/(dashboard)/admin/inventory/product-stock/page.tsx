...
export default function ProductStockPage() {
  ...
  const handleOpenProductAdjust = (product: Product, stockRecord?: StockRecord) => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-in fade-in duration-500">
          {filteredProducts.map((product) => {
            const stock = product.stocks?.find(s => s.size === 'M') || product.stocks?.[0];
            const totalStock = stock ? stock.quantity : 0;
            const totalReserved = stock ? stock.reserved_quantity : 0;
                    ...
                    <div className="border border-zinc-100 rounded-2xl overflow-hidden shadow-sm bg-white">
                      <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tbody className="divide-y divide-zinc-100 text-zinc-750 font-bold">
                          {(() => {
                            const avail = stock.quantity - stock.reserved_quantity;
                            const isLow = avail <= stock.low_stock_threshold;
                            const isOut = avail <= 0;
                            ...
                            return (
                              <tr className={`hover:bg-zinc-50/50 transition-colors ${
                                ...
                                <td className="py-2.5 px-4 font-mono">{stock.quantity}</td>
                                <td className="py-2.5 px-4 text-right font-mono text-zinc-400">{stock.reserved_quantity}</td>
                                <td className={`py-2.5 px-4 text-right font-black font-mono ${
                                  isOut ? 'text-red-650' : isLow ? 'text-amber-650' : 'text-emerald-600'
                                }`}>{avail}</td>
                                <td className="py-2.5 px-4 text-right font-mono text-zinc-400">≤ {stock.low_stock_threshold}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          ...
          >
            ...
            <form onSubmit={handleAdjustSubmit} className="mt-6 space-y-6">
              ...
              {/* Stats */}
              {(() => {
                const currentStock = selectedProduct.stocks.find(s => s.size === 'M') || selectedProduct.stocks[0];
                return (
