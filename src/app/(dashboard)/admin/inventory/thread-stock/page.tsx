...
export default function ThreadStockPage() {
  ...
  const fetchStockData = async () => {
    ...
    try {
      ...
      calculateStats(threadsData);
    } catch (err: any) {
      console.error(err);
      toast.error('Failed to load thread stock catalog.');
    } finally {
      setIsLoading(false);
    }
  };

  const calculateStats = (threadList: Thread[]) => {
    let lowStock = 0;
    let outOfStock = 0;

    threadList.forEach(thr => {
      const qty = Number(thr.quantity) || 0;
      const threshold = Number(thr.low_stock_threshold) || 0;
      if (qty <= 0) {
        outOfStock++;
  ...
  const handleOpenThreadAdjust = (thread: Thread) => {
    setSelectedThread(thread);
    setPhysicalDelta(0);
    setNewThreshold(Number(thread.low_stock_threshold) ?? 10);
  ...
  const handleAdjustSubmit = async (e: React.FormEvent) => {
    ...
    const targetName = selectedThread.name;
    ...
    try {
      ...
      toast.success('Thread stock adjusted successfully!', { id: loadingToast });
  ...
  const filteredThreads = React.useMemo(() => {
    return threads.filter(thr => {
      const matchesSearch = 
        ...
        thr.code.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (thr.type || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (thr.description || '').toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black italic tracking-tighter text-[#3a525d]">Thread Stock</h1>
          </div>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#2d8d9b] mt-2 opacity-80">
            Spools, nylon, polyester thread levels & custom stock alerts
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-[2rem] border border-[#fce4d4] p-5 shadow-sm flex items-center gap-4 hover:border-[#2d8d9b]/30 transition-all duration-300 relative overflow-hidden group">
          ...
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest text-[#2d8d9b]/70">Thread Categories</p>
            <h3 className="text-2xl font-black italic tracking-tight text-[#2d8d9b] mt-0.5">{stats.totalThreads}</h3>
          </div>
        ...
        <div className="bg-white rounded-[2rem] border border-[#fce4d4] p-5 shadow-sm flex items-center gap-4 hover:border-red-200 transition-all duration-300 relative overflow-hidden group">
          ...
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest text-red-600/70">Depleted Spools</p>
      ...
      <div className="bg-white border border-[#fce4d4] rounded-[2rem] p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
        ...
        <div className="relative group w-full md:w-96">
          ...
          <input 
            ...
            placeholder="Search by code, name, type, details..."
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center p-24 bg-white border border-[#fce4d4] rounded-[3rem] gap-4">
          ...
          <p className="text-sm font-bold text-[#3a525d]">Parsing threads database...</p>
        </div>
      ) : filteredThreads.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-in fade-in duration-500">
          {filteredThreads.map((thread) => {
            const qty = Number(thread.quantity) || 0;
            const threshold = Number(thread.low_stock_threshold) || 0;
            ...
            return (
              <div 
                key={thread.id}
                onClick={() => handleOpenThreadAdjust(thread)}
              ...
              >
                <div>
                  ...
                  <div className="flex justify-between items-start gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-black text-sm text-[#3a525d] tracking-tight line-clamp-1">{thread.name}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-5 bg-white/40 backdrop-blur-sm rounded-2xl p-3 border border-black/5 text-center">
                    <div>
                      <p className="text-[7.5px] font-black uppercase tracking-widest text-zinc-400">Type</p>
                      <p className="text-[10px] font-black text-[#3a525d] truncate mt-0.5">{thread.type || 'Standard'}</p>
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] font-black text-[#3a525d] truncate mt-0.5">
                        {thread.unit_price ? `₹${Number(thread.unit_price).toFixed(2)}` : 'N/A'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 space-y-2">
                    <div className="flex justify-between items-end text-xs">
                      <div>
                        <p className="text-[8px] font-black uppercase tracking-wider text-zinc-400">Spools Available</p>
                        <p className="text-xl font-black text-[#3a525d] italic tracking-tight">{qty.toFixed(0)} <span className="text-xs font-bold not-italic text-zinc-500">spools</span></p>
                      </div>
                      <div className="text-right">
                        <p className="text-[8px] font-black uppercase tracking-wider text-zinc-400">Low Stock Limit</p>
                        <p className="text-xs font-extrabold text-zinc-600">≤ {threshold.toFixed(0)} spools</p>
                      </div>
                    </div>
                ...
                <div className="mt-4 pt-3 border-t border-black/5 flex justify-between items-center text-[9px] font-black uppercase tracking-wider text-zinc-400">
                  <span>Press to adjust thread stock</span>
        ...
        <div className="bg-white border border-[#fce4d4] rounded-[3rem] p-24 text-center">
          <div className="flex flex-col items-center gap-4 opacity-30">
            <FolderKanban size={48} className="text-[#2d8d9b]" />
            <p className="text-xl font-black italic text-[#3a525d]">No matching thread records found</p>
            <p className="text-xs font-bold max-w-sm text-zinc-500">Try adjusting your filters or search keywords to locate specific threads.</p>
          </div>
        </div>
      )}

      {/* Adjust Stock & Threshold Modal */}
      {isAdjustModalOpen && selectedThread && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-zinc-100 pb-5 mt-2">
              ...
              <div>
                <h3 className="text-xl font-black italic text-[#3a525d] tracking-tight">Adjust Thread Stock</h3>
            ...
            <form onSubmit={handleAdjustSubmit} className="mt-6 space-y-6">
              ...
              {/* Stats */}
              {(() => {
                const currentQty = Number(selectedThread.quantity) || 0;
                const lowThreshold = Number(selectedThread.low_stock_threshold) || 0;
                return (
                  <div className="bg-zinc-50 rounded-2xl p-4 border border-zinc-100 grid grid-cols-2 gap-3 text-center">
                    <div>
                      <p className="text-[8px] font-black uppercase tracking-widest text-zinc-400">Current Thread Stock</p>
                      <p className="text-base font-black text-[#3a525d] mt-1">{currentQty.toFixed(0)} spools</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-black uppercase tracking-widest text-zinc-400">Threshold limit</p>
                      <p className="text-base font-black text-[#2d8d9b] mt-1">{lowThreshold.toFixed(0)} spools</p>
                    </div>
                  </div>
                );
              })()}
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-[#3a525d]">
                  ...
                  <span className={`text-[9px] font-extrabold ${physicalDelta > 0 ? 'text-emerald-500' : physicalDelta < 0 ? 'text-red-500' : 'text-zinc-400'}`}>
                    {physicalDelta > 0 ? `+${physicalDelta} spools` : physicalDelta < 0 ? `${physicalDelta} spools` : 'No physical adjustment'}
                  </span>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] font-black text-[#2d8d9b] bg-[#2d8d9b]/5 px-2 py-0.5 rounded">
                    Current limit: {newThreshold} spools
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <input
                ...
                <p className="text-[9px] text-zinc-400 font-bold">
                  When available spool physical count falls below this limit, it flags an alert.
                </p>
              </div>
