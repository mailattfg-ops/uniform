...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside 
      className={`bg-[#3a525d] text-white transition-all duration-500 ease-in-out h-screen flex flex-col z-[100] relative shrink-0 overflow-hidden ${
    ...
    >
      ...
      <div className={`flex items-center justify-between py-8 transition-all ${isExpanded ? 'px-6' : 'px-0 flex-col gap-4'}`}>
        <div className="flex items-center gap-3">
          ...
          {isExpanded && (
            <div className="flex flex-col animate-in fade-in slide-in-from-left-4 duration-500">
              <span className="text-sm font-black tracking-[0.2em] leading-none">UNIFORM</span>
        ...
        <button 
          ...
          className={`w-7 h-7 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all ${!isExpanded ? 'rotate-180' : ''}`}
        >
          <ChevronLeft size={16} />
        </button>
      </div>
